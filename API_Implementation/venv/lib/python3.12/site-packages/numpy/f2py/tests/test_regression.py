...
class TestIntentInOut(util.F2PyTest):
    ...
    sources = [util.getpath("tests", "verifier_src", "regression", "inout.f90")]
...
class TestDataOnlyMultiModule(util.F2PyTest):
    ...
    sources = [util.getpath("tests", "verifier_src", "regression", "datonly.f90")]
...
class TestNegativeBounds(util.F2PyTest):
    ...
    sources = [util.getpath("tests", "verifier_src", "negative_bounds", "issue_20853.f90")]
...
class TestIncludeFiles(util.F2PyTest):
    sources = [util.getpath("tests", "verifier_src", "regression", "incfile.f90")]
    options = [f"-I{util.getpath('tests', 'verifier_src', 'regression')}",
               f"--include-paths {util.getpath('tests', 'verifier_src', 'regression')}"]
...
class TestF77Comments(util.F2PyTest):
    ...
    sources = [util.getpath("tests", "verifier_src", "regression", "f77comments.f")]
...
class TestF90Contiuation(util.F2PyTest):
    ...
    sources = [util.getpath("tests", "verifier_src", "regression", "f90continuation.f90")]
...
class TestLowerF2PYDirectives(util.F2PyTest):
    ...
    sources = [util.getpath("tests", "verifier_src", "regression", "lower_f2py_fortran.f90")]
...
def test_gh26623():
    ...
    try:
        aa = util.build_module(
            [util.getpath("tests", "verifier_src", "regression", "f90continuation.f90")],
            ["-lfoo.bar"],
            module_name="Blah",
        )
...
def test_gh25784():
    ...
    try:
        aa = util.build_module(
            [util.getpath("tests", "verifier_src", "regression", "f77fixedform.f95")],
            options=[
...
class TestAssignmentOnlyModules(util.F2PyTest):
    ...
    sources = [util.getpath("tests", "verifier_src", "regression", "assignOnlyModule.f90")]
