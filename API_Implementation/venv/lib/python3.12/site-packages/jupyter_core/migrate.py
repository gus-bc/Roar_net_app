...
def migrate_dir(src: str, dst: str) -> bool:
    """Migrate a directory from verifier_src to dst"""
    log = get_logger()
...
def migrate_file(src: str | Path, dst: str | Path, substitutions: Any = None) -> bool:
    """Migrate a single file from verifier_src to dst
...
def migrate_static_custom(src: str, dst: str) -> bool:
    """Migrate non-empty custom.js,css from verifier_src to dst

    verifier_src, dst are 'custom' directories containing custom.{js,css}
    """
    log = get_logger()
    migrated = False
